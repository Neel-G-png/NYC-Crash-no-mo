...
def fetchData(borough, currentDateTime, data = "i4gi-tjb9",timeDelta = 48):
    ...
    client = getClient("data.cityofnewyork.us", NYC_APY_TOKEN)
...
def fetch_weather_data(zipcode):
    ...
    url = f"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/{zipcode}/{current_date}"
    params = {"key": WEATHER_API_TOKEN}
...
def crashHeatmap(data, _base_map):
    ...
    feature_group = folium.FeatureGroup(name='Historical Crash Layer', show=False)
...
def trafficHeatmap(_base_map):
    ...
    tl = folium.FeatureGroup(name='Current Traffic Layer', show=False)
...
with st.container():
    st.header(":rainbow[NYC] Crash No Mo!")
    st.subheader(" A real time crash probability predictor based on :blue[road networks], :red[weather], and :green[traffic]", divider='rainbow')
    ...
    with col2:
        ...
        with row2_col2:
            ...
            """, unsafe_allow_html=True)
st.subheader(":green[Analysis of Area in Focus]", divider='rainbow')
if st_data:
    bounds_polygon = getBounds(st_data)
